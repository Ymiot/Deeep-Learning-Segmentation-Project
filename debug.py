...
# -------------------------
BASE_SAVE_PATH = "/zhome/3f/9/223204/tmp_training"
CHECKPOINTS_DIR = os.path.join(BASE_SAVE_PATH, "checkpoints")
PLOTS_DIR = os.path.join(BASE_SAVE_PATH, "plots")
...
def main():
    ...
    for epoch in range(cfg["epochs"]):
        ...
        if avg_val["dice"] > best_val_dice:
            ...
            torch.save(model.state_dict(), f"{CHECKPOINTS_DIR}/{args.model}_{args.dataset}_best.pt")
            print(f"  -> New best model saved at {CHECKPOINTS_DIR}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{PLOTS_DIR}/loss_curves.png")
    plt.show()
    ...
    for metric in train_metrics_history.keys():
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f"{PLOTS_DIR}/{metric}_curves.png")
        plt.show()
